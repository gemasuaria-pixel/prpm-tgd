<div class="card border-0 shadow rounded-4">
    <div class="card-body p-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h5 class="fw-bold text-primary mb-0">{{ $laporan->judul ?? '-' }}</h5>
        ...
        <div class="row g-3 mb-3">
            <div class="col-md-6 small">
                ...
                <p><strong>Rumpun Ilmu:</strong> {{ $laporan->proposalPengabdian->rumpun_ilmu ?? '-' }}</p>
                <p><strong>Bidang:</strong> {{ $laporan->proposalPengabdian->bidang_pengabdian ?? '-' }}</p>
            </div>
            <div class="col-md-6 small">
        ...
        <div class="mt-4">
            ...
            <form wire:submit.prevent="updateStatus" class="needs-validation" novalidate>
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select wire:model="status" id="status" class="form-select"
                            {{ $allApproved || $laporan->status === 'final' ? 'disabled' : '' }}>
                            @if (!$allApproved)
                                <option class=" text-muted" value=""  selected>-- Pilih Status --</option>
                            ...
                            <template x-for="(rev, i) in selectedReviewers" :key="rev.id">
                                <span class="badge bg-primary px-2 py-1 d-inline-flex align-items-center">
                                    ...
                                    <button type="button" class="btn-close btn-close-white ms-2"
                                        ...
                                        {{ $laporan->status === 'final' || $allApproved ? 'disabled' : '' }}></button>
                                </span>
                            </template>
                ...
                <div class="d-flex justify-content-between mt-4">
                    <a wire:navigate href="{{ route('ketua-prpm.review.pengabdian.laporan.index') }}"
                        class="btn btn-light border">
                        <i class="bi bi-arrow-left me-1"></i> Kembali
                    </a>

                    @if ($allApproved && $laporan->status !== 'final')
                        <button type="button" wire:click="updateStatus" class="btn btn-primary px-4">
                            <i class="bi bi-check-circle me-1"></i> Finalkan Laporan
                        </button>
                    @endif
