<div>
    ...
    <ul class="nav nav-tabs mt-3 px-3">
        <li class="nav-item">
            <a href="{{ route('ketua-prpm.review.penelitian.proposal.index') }}" wire:navigate
                class="nav-link {{ request()->routeIs('ketua-prpm.review.pengabdian.proposal.index') ? 'active' : '' }}">
                Proposal
            </a>
        </li>
        <li class="nav-item">
            <a href="{{ route('ketua-prpm.review.penelitian.laporan.index') }}" wire:navigate
    ...
    <table class="table table-hover table-sm align-middle">
        ...
        <tbody>
            @forelse ($laporans as $index => $entry)
                <tr wire:key="proposal-{{ $entry->id }}">
                    <td>{{ $index + $laporans->firstItem() }}</td>
                    <td>{{ Str::limit($entry->judul, 40) }}</td>
                    <td>{{ $entry->proposalPenelitian->ketuaPengusul->name ?? '-' }}</td>
                    <td>{{ $entry->proposalPenelitian->rumpun_ilmu ?? '-' }}</td>
                    <td>
                        @php
                    ...
                    <td class="text-center">
                         <a wire:navigate
                            href="{{ route('ketua-prpm.review.penelitian.laporan.form', ['laporanId' => $entry->id]) }}"
